...
class SyscallTracer(Application):
    ...
    def get_sys_call_distributed(self):
        ...
        while queue:
            ...
            self.printf(f'Current node is {current_node}, Current func is {func_name}')
                ...
                if current_node == current_graph_out_node:
                    if current_node != self.actual_out_node:
                        ...
                        if p_node is not None:
                            ...
                            self.printf(f'\tOut node {current_node} detected checking its previous graph..')
                            queue.append((p_node, p_name, p_graph, p_succ))
                            self.printf(f'\tAdded previous graph node {p_node} and its successors..')
                            visited_nodes.add(current_node)
                            self.printf(f'\tRemoving {current_node} as no more child')
                            queue.popleft()
                    else:
                        self.printf(f'Encountered final out node of graph Removing {current_node}')
                        valid_sys_calls_data['exit_group_syscall'] = None
                        queue.popleft()
                else:
                    try:
                        child = next(children)
                        self.printf(f'\tProcessing child :- {child}')
                        self.printf(f'\tCurrent Path traversed is {current_node}->{child}')
                        label_data = graph_data[current_node][child]['label']
                        if label_data.endswith('syscall'):
                            self.printf(f'\tSystem call {label_data} obtained')
                            ...
                            self.printf(f'\tAdded {child}, {func_name} to self.path_data of {child}')
                        elif (label_data == 'ep'):
                            self.printf(f'\tAdding child {child} and its successors for processing..')
                            ...
                            for in_node in in_nodes:
                                ...
                                self.printf(f'\tAdding reference to {in_node} of func {label_data}')
                                if self.path_data.get(in_node, None) is None:
                                    self.path_data[in_node] = {}
                                self.printf(f'\tAdded {child}, {func_name} to self.path_data of {in_node}')
                        ...
                        self.printf(f'Removing current node {current_node} as all children processed..')
                        visited_nodes.add(current_node)
                        queue.popleft()
                    except Exception as e:
                        self.printf(f'{e}')
            else:
                self.printf(f'\t Already visited {current_node}, removing from queue')
                queue.popleft()
    ...
    def parseOptions(self):
        ...
        parser.add_option("-i", "--show-ip",
                          ...
                          action="store_true", default=False)
        parser.add_option("--graph-path", '-g',
    ...
    def displaySyscall(self, syscall):
        print(f'Encountered syscall {syscall.name}')
        if syscall.name in self.ignore_sys_calls_list:
            pass
        else:
            to_check = f'{syscall.name}_syscall'
            start = time()
            valid_sys_call_dict = self.get_sys_call_distributed()
            print(time() - start)
            valid_sys_calls = set(valid_sys_call_dict.keys())
            print(valid_sys_calls)
            if to_check not in valid_sys_calls:
                ...
                if to_check.startswith('exit'):
                    ...
                    node_data = valid_sys_call_dict[to_check]
                ...
                for key in list(self.path_data.keys()):
                    if key not in self.in_nodes:
                        self.printf(f'Removing {key}')
                        del self.path_data[key]
                self.printf(f'Nodes to traverse next are {self.in_nodes} in {self.out_funcs} and their back_track_data {self.path_data}')
